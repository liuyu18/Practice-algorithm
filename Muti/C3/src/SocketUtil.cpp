...
UDPSocket SockrtUtil::CreateUDPSocket(SocketAddressFamily inFamily){
    SOCKET s = socket(inFamily,SOCKET_DGRAM,IPPROTO_UDP);
    if(s != INVALID_SOCKET){
...
void SocketUtil::FillVectorFromSet( vector< TCPSocketPtr >* outSockets, const vector< TCPSocketPtr >* inSockets, const fd_set& inSet ){
    if(inSockets && outSockets){
        outSockets->clear();
        for(const TCPSokcetPtr &socket : *inSockets){
...
int SocketUtil::Select( const vector< TCPSocketPtr >* inReadSet,
					   ...
					   vector< TCPSocketPtr >* outExceptSet ){
                           ...
                           int nfds = 0;
                            ...
                            int ToRet = select(nfd + 1,readPtr,writePtr,exceptPtr,nullptr);
                            if(toRet > 0){
